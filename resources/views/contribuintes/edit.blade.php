...
<form class="space-y-8" id="ipraForm" method="POST" action="{{ route('contribuintes.validar.update', ['ipraForm' => $ipraForm->id]) }}">
    @csrf
    @method('PATCH')
<!-- Escolha do Tipo -->
  <div class="mb-6">
  ...
  <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div class="sm:col-span-3">
        ...
        <textarea class="w-full rounded-lg border p-2 @error('observacoes') border-red-500 ring-1 ring-red-500 @else border-blue-300 @enderror" name="observacoes">{{ old('observacoes', $dados['observacoes'] ?? '') }}</textarea>
        @error('observacao')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
