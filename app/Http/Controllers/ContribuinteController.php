...
class ContribuinteController extends Controller
{
    ...
    {
        $this->authorize('viewAny', IpraForm::class);
 ...
 // Tipo de pessoa vem do form (ex: 'singular' ou 'colectiva')
    ...
    if ($tipoPessoa === 'Singular') {
       ...
       public function validarUpdate(Request $request, IpraForm $ipraForm)
    {
 // Tipo de pessoa vem do form (ex: 'singular' ou 'colectiva')
    $tipoPessoa = $request->input('tipoPessoa');
    ...
    if ($tipoPessoa === 'Singular') {
        ...
        // Redireciona para a tela de confirmação
        return redirect()->route('contribuintes.confirmarUpdate', ['ipraForm' => $ipraForm->id]);
    }
    ...
    public function confirmar(Request $request)
    {
        ...
        return view('contribuintes.confirmar', compact('modo','dados','dataToShow', 'Ae', 'P', 'Fl', 'Fa', 'Al', 'taxaUso', 'Vp', 'ipra', 'finalidadeImovel'));
    }
   ...
   public function limpar()
{
    ...
    {
        $this->authorize('excelExport', IpraForm::class);
        return Excel::download(new IpraFormsExport, now()->format('Y-m-d_H-i-s').'-ipraforms.xlsx');
    }

    public function history(IpraForm $ipraForm)
    {
        // $logs = DB::table('ipra_form_logs')
        //     ->where('ipra_form_id', $ipraForm->id)
        //     ->orderBy('created_at', 'desc')
        //     ->get();
        $this->authorize('history', $ipraForm);
        $logs = $ipraForm->logs()->latest()->get();


        return view('contribuintes.history', compact('ipraForm', 'logs'));
    }

    public function edit( IpraForm $ipraForm)
    {

        $this->authorize('update', $ipraForm);
        $dados = \App\Helpers\ConversorHelper::ipraFormToData($ipraForm);
        ...
        return view('contribuintes.edit', compact('ipraForm','postosAdministrativos','tiposDocumentoIdentificacao', 'zonas', 'classesImovel', 'finalidadesImovel', 'dados', 'bairros'));
    }

    public function confirmarUpdate(Request $request, IpraForm $ipraForm)
    {
        $this->authorize('update', $ipraForm);
        $dados = session('ipra_dados');
        $dataToShow = $dados;
        ...
        if (!$dados) {
            return redirect()->route('contribuintes.edit', ['ipraForm' => $ipraForm->id])->with('error', 'Nenhum dado encontrado para confirmar.');
        }



        return view('contribuintes.confirmar', compact('modo','ipraForm','dados','dataToShow', 'Ae', 'P', 'Fl', 'Fa', 'Al', 'taxaUso', 'Vp', 'ipra', 'finalidadeImovel'));
    }


    public function update(Request $request, IpraForm $ipraForm)
    {
        $this->authorize('update', $ipraForm);
        $dados = session('ipra_dados');
        $saveData['tipo_pessoa'] = $dados['tipoPessoa'];
        ...
        $saveData['user_id'] = auth()->id();
         ...
         $ipraForm->update($saveData);
        // Model::create($dados);
        session()->forget('ipra_dados');
        return redirect()->route('contribuintes.edit', ['ipraForm' => $ipraForm->id])->with('success', 'Dados salvos com sucesso!');
    }

    public function resumoPDF(Request $request, IpraForm $ipraForm)
    {
        $this->authorize('print', $ipraForm);
        //Ipra calculation
        $Ae = $ipraForm->area_construida; //Area edificada
        $P = $ipraForm->classeImovel->preco_m2; // Preco medio por Metro quadrado
        $Fl = $ipraForm->zona->fator_localizacao; // Factor de Localizacao
        ...
        $pdf = Pdf::loadView('reports.resumo', [
            'ipra' => $ipraForm,
            'P' => $P,
            'Fa' => $Fa,
            'Fl' => $Fl,
        ...
        return $pdf->stream(now()->format('Y-m-d_H-i-s')."-ipra-{$ipraForm->id}.pdf");
    }
